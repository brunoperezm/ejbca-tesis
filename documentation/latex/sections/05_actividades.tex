...
Se descargó la imagen de \textit{VMware ESXi} 6.7\cite{VMware_6.7_download} y se montó en un pendrive usando \textit{Rufus}\cite{Rufus}. Se inició el servidor con el pendrive insertado, además de pantalla y teclado, y se forzó a usar el pendrive para bootear, consiguiendo acceder a la pantalla de instalación de \textit{VMware}. % revisar

Para instalar, se siguieron procedimientos oficiales\cite{VMware_6.7_install}, eligiendo el disco duro del servidor, borrando todo su contenido preexistente.

Una vez que se reinició y levantó con el nuevo SO, se le configuró una IP privada provisional, para poder entrar vía web a la gestión del servidor. Para ello, se accedió a: \texttt{F2 $\rightarrow{}$ Configure Management Network $\rightarrow{}$ IPv4 Configuration} % revisar
...
Habiendo ya configurado una IP, y conectando una computadora directamente al NIC 0, se pudo ingresar poniendo la IP en la URL del navegador, iniciando con usuario root y contraseña configurada durante el proceso de instalación, accediendo a la página inicial. En esta página se pudo ver información sobre los componentes del servidor. % revisar
...
Teniendo ya el virtualizador, se comenzó con configuraciones generales, antes de empezar con las VMs. % revisar

Se habilitó la opción que permite seleccionar VMs para que se enciendan automáticamente cuando el servidor se encienda, para ello se accedió a \texttt{Host $\rightarrow{}$ Administrar $\rightarrow{}$ Inicio automático} y se habilitó con un retraso de 120 segundos al iniciar. % revisar

Desde allí se asignó la licencia para poder hacer uso de todas las opciones del sistema desde \texttt{Host $\rightarrow{}$ Administrar $\rightarrow{}$ Licencias $\rightarrow{}$ Asignar licencia} % revisar

A continuación se realizaron los preparativos para poder cumplir con la topología de red. Para ello se accedió a \texttt{Redes $\rightarrow{}$ Conmutadores virtuales $\rightarrow{}$ Agregar conmutador virtual estándar}. Allí se crearon dos switch virtuales, ligando cada uno a uno de los NIC físicos del equipo. % revisar
...
A continuación, en \texttt{Redes $\rightarrow{}$ Grupos de puertos $\rightarrow{}$ Agregar grupos de puertos} se crearon las VLANs asignándolas al switch virtual correspondiente.
...
Como último paso en la preparación previa a la creación de las VMs, se subió la imagen del \textit{Ubuntu Server} 20.04\cite{Ubuntu_20.04_download} a utilizar en \texttt{Almacenamiento $\rightarrow{}$ datastore1 $\rightarrow{}$ Explorador de almacenes de datos $\rightarrow{}$ Cargar}. % revisar

Una vez que se tuvo todo el escenario preparado, en \texttt{Máquinas virtuales $\rightarrow{}$ Crear/Registrar máquina virtual} se pudieron crear las VMs. % revisar
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item A todas las VMs se les asignó 2 vCPU. % revisar
        \end{itemize}
        \item Memoria $\rightarrow{}$ Memoria tipo RAM
        \begin{itemize}
            \item La memoria era el recurso más escaso del servidor, por lo que se debió asignar responsablemente sus recursos % revisar
            \item A la VM \textit{Bastion} y \textit{destino1} se les asignó 1GB, dado que solamente recibirían solicitudes de acceso. % revisar
            \item A la VM de \textit{EJBCA} se le asignó 2GB, ya que necesita \href{https://hub.docker.com/r/keyfactor/ejbca-ce#:~:text=Minimum%20System%20Requirements,at%20least%201GB%20of%20RAM}{1GB para correr el contenedor}, y se le asignó 1GB extra para el servicio de autenticación. % revisar
        ...
        \begin{itemize}
            \item A todas las VMs se les asignó 35GB. % revisar
        \end{itemize}
...
Al terminar con esta tarea, las 3 VMs fueron creadas, al seleccionarlas en Máquinas virtuales, se abrió la información detallada de cada una. % revisar
...
En este punto, ya se pudieron encender las VMs, y configurarlas haciendo uso de la consola que se mostró en su página dedicada. Adicionalmente se tomó la precaución de habilitar el inicio automático en caso de reinicio seleccionando \texttt{Acciones $\rightarrow{}$ Inicio automático $\rightarrow{}$ Habilitar}. % revisar


Para instalar \textit{Ubuntu} sobre las VMs, se pudo seguir un procedimiento estándar\cite{instalar_ubuntu}, teniendo en consideración: % revisar
\begin{itemize}
    \item En la página dedicada a cada VM donde se encontró la consola, también se pudo encontrar en \textit{Configuración de hardware} los adaptadores de red asignados, y al expandirlos, se mostró la MAC de cada uno. % revisar
    \item Haciendo uso de la MAC de cada interfaz de red, en la etapa de configuración de redes se pudieron configurar las distintas redes según se detalló en Topología de Red \ref{sec:topología_de_red} % revisar
    \item Se hizo uso del disco entero, sin usar LVM ni RAID. % revisar
    \item Se utilizó la opción de instalar \textit{OpenSSH} durante la instalación del SO. % revisar
    \item No se hizo uso de la opción de instalar paquetes adicionales. % revisar
\end{itemize}

\subsection{Conclusión}

Al finalizar con esta iteración, se contó con un servidor \textit{Dell} con SO \textit{VMware ESXi} 6.7, donde se configuraron dos switch virtuales con distintas VLANs en los mismos, y 3 VMs con SO \textit{Ubuntu Server} 20.04, con inicio automático configurado. % revisar
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Se hizo uso de un servidor, el cual está por definición preparado para funcionar constantemente sin interrupción\cite{server_vs_pc}, y haciendo uso de la opción de inicio automático, se preparó al sistema para otorgar la mayor disponibilidad disponible. % revisar
    \end{itemize}
...
Para empezar esta iteración, se comenzó realizando la instalación de \textit{Docker} y \textit{Docker-Compose}: % revisar
...
En este servidor no se instaló de manera local \textit{SoftHSM}, sin embargo fue necesario conocer cómo instalarlo y usarlo, para luego poder sincronizarlo con \textit{EJBCA}, y a su vez para investigar su posible uso del lado del técnico. % revisar

Para instalar \textit{SoftHSM}, se pudo realizar con: % revisar
\begin{lstlisting}[language=Bash, caption={Instalación SoftHSM}, label={cod:bash0}, captionpos=b]
sudo apt-get install softhsm2
\end{lstlisting}


El paquete de \textit{Ubuntu} para \textit{SoftHSM2} no siempre se inicializa correctamente (según la versión del SO), por lo que fue posible que se debieran crear directorios faltantes, etc. También pudieron existir problemas con permisos, por ello se presentaron los siguientes pasos, siendo el último el único obligatorio, que es el comando con el que se inicia un slot: % revisar
...
Reemplazando donde dice \textbf{NAME} por el nombre que se consideró oportuno asignar: % revisar
...
En la imagen se puede observar cómo solicitó insertar la contraseña del usuario del sistema y luego pidió establecer un PIN que fue la contraseña del slot. Tanto el LABEL como el PIN fueron de importancia para la manipulación del \textit{SoftHSM}. % revisar
Para comprobar los slots creados, se pudieron verificar con: % revisar
...
En caso de tener problemas con la instalación manual, también existieron alternativas, por ejemplo repositorios \href{https://github.com/sungsu-cho/softhsm-install}{\textit{GitHub}}, donde la instalación está resuelta (en este caso, junto a \textit{OpenSSL}), o similares. % revisar


\subsubsection{Instalación de EJBCA con Docker y SoftHSM}
\label{sec:instalacion_ejbca_cocker_softhsm}

Anteriormente se mencionó que para levantar el servicio de \textit{EJBCA}, se tomó la decisión de utilizar el \textit{Docker} ofrecido en su versión gratuita por \textit{Bitnami}, y si bien existía la posibilidad de utilizar las versiones estables en \textit{DockerHub}, se eligió descargar el \href{https://github.com/bitnami/containers/tree/main/bitnami/ejbca}{repositorio oficial} que se pudo encontrar abierto en \textit{GitHub}, esto se hizo para poder realizar modificaciones. % revisar

Por lo tanto, dentro de la carpeta \texttt{/home/ejbca} se descargó el repositorio, y dentro de \texttt{/xx/8/debian-12} se encontró su \textit{Dockerfile} y \textit{docker-compose}. % revisar

En el \textit{docker-compose} se pudo ver cómo incluye la imagen de la base de datos \textit{MariaDB} y la imagen de \textit{EJBCA}, por lo que para incluir \textit{SoftHSM} se presentaron 3 opciones: % revisar

\begin{enumerate}
    \item Realizar una tercera imagen con \textit{SoftHSM} instalado, y comunicarla con la imagen de \textit{EJBCA} mediante volúmenes compartidos.
    \item Tener \textit{SoftHSM} instalado de manera local, y comunicarlo con el contenedor para su uso.
    \item Instalar \textit{SoftHSM} en la misma imagen de \textit{EJBCA}
\end{enumerate}

Con respecto a las primeras dos opciones, se encontró el detalle de que \textit{EJBCA} solo permitía hacer uso del \textit{SoftHSM} si el mismo se encontraba instalado en el mismo usuario, lo que se presentó como un impedimento, que fue resuelto ya por \textit{Bitnami} en su versión paga, por lo que se descartó dicha opción. % revisar

Justificando de esta manera el haber descargado el repositorio, se optó por la tercera opción, y se modificó el \textit{Dockerfile}, agregando sobre el final las siguientes líneas: % revisar
...
Y de esta manera, al levantar el contenedor, se instaló también dentro del mismo \textit{SoftHSM} (haciendo uso de un comprimido con un \textit{Makefile} con los pasos necesarios) y fue posible desde la interfaz de \textit{EJBCA} acceder al slot creado. % revisar

Volviendo al \textit{docker-compose}, el mismo debió ser modificado también. % revisar 

Primero, en las variables de ambiente, tanto para \textit{MariaDB} como \textit{EJBCA}, se presentaron opciones de personalización tales como usuario y contraseña, que fuera de un entorno de investigación deberían haber sido modificadas y ocultadas. % revisar

A continuación se pudo notar que la imagen de \textit{EJBCA} que utilizaba era la que se encontraba en \textit{DockerHub}, por lo tanto se cambió la línea para que utilizara el \textit{Dockerfile} recientemente modificado, de forma que: % revisar
...
Adicionalmente, para asegurarse de que ante un reinicio de sistema, los contenedores \href{https://www.baeldung.com/ops/docker-containers-start-automatically}{reiniciaran automáticamente}, se agregó al final tanto de la descripción de \textit{MariaDB} como de \textit{EJBCA}: % revisar

\begin{lstlisting}[language=Bash, caption={Reinicio automatico de Docker}, label={cod:restart_always}, captionpos=b]
restart: always
\end{lstlisting}

Con todo ya preparado para iniciar los contenedores, y utilizando un \textit{Dockerfile} en lugar de una imagen de \textit{DockerHub}, ubicados en la carpeta con ambos archivos, se utilizaron dos comandos: % revisar
...
En este punto, se habilitó para configurar y operar \textit{EJBCA} una página web, para poder acceder a ella, se podría haber conectado al servidor mediante un túnel, o configurar en la VLAN 5 otra salida fuera del servidor para tener acceso vía LAN, sin embargo se aprovechó la conexión temporal que le permitía acceder a internet, que se encontraba en la misma red que los equipos, para ser más precisos, la IP \texttt{172.18.66.221}. % revisar

Se comenzó descargando el certificado de administrador que permitiría acceder a la admin web donde realizar las configuraciones necesarias. Para ello se accedió a \texttt{https://172.18.66.221:8443/ejbca/ra/enrollwithusername.xhtml}, y como se puede observar en la imagen \ref{fig:superadmin_enroll} pidió credenciales. % revisar
...
Como no se cambiaron en el \textit{Docker-Compose} las credenciales por defecto, se ingresaron en este punto siendo \textit{superadmin} de usuario, y \textit{Bitnami1234} de contraseña. % revisar
A continuación preguntó el algoritmo deseado para el certificado, se seleccionó RSA-2048, y finalmente la opción para descargar el PKCS\#12. % revisar

Con eso se descargó un archivo de extensión \texttt{.p12}. % revisar
Este archivo fue el certificado del \textit{superadmin} que se tuvo que ingresar en el navegador. Para hacerlo en \textit{Firefox}: % revisar
...
En el navegador se puso la URL \texttt{https://172.18.66.221:8443/ejbca/adminweb/} y se pudo acceder a la página de administración, donde se tuvieron al alcance todas las opciones de configuración para configurar un PKI y poder emitir certificados. % revisar
...
Al usar el comando, nos preguntara por ubicación donde guardarlos, y también si queremos una contraseña, ambas son opcionales, lo recomendable es que tenga una contraseña, y que la ubicación se encuentre en \texttt{\textasciitilde{}/.ssh/}
...
En este ejemplo, no hubo necesidad de especificar la ubicación de nuestro par de claves, dado que usamos el nombre y ubicación por defecto. Si quisiéramos especificar la ubicación de mi clave pública, se puede usar el modificador \texttt{-I \char`\<Ubicación de mi archivo\char`\>}
, por ejemplo, para utilizar mi clave generada en el SoftHSM, el comando a utilizar es:
...
\begin{itemize}
    \item \textbf{REQ\_F\_SYA\_2:}
    \begin{itemize}
        \item Se cumple este requerimiento de dos formas, la primera al generar un usuario en el servidor destino para el técnico, o por rol según se decidiera, distinto al de root al que se pueden conectar. La segunda es con la creación de un usuario de salto para Bastion.
    \end{itemize}
...
\begin{itemize}
    \item \textbf{REQ\_F\_SYA\_2:}
    \begin{itemize}
        \item Al configurar el modulo PAM para actuar en el directorio del usuario que solicita acceso, permite que el acceso del técnico sea con el usuario que corresponda, y evita que por problemas de escalabilidad se termine resolviendo que se utilice el usuario root para dar acceso a los técnicos.
    \end{itemize}
    \item \textbf{REQ\_F\_SYA\_7:}
    \begin{itemize}
        \item Configurando la sesión para borrar las credenciales de usuario cuando el mismo se desconecta, el mismo deberá nuevamente pasar por el proceso completo para poder ingresar.
    \end{itemize}
    \item \textbf{REQ\_F\_IUS\_4:}
    \begin{itemize}
        \item Se cumple el requerimiento habiendo especificado el ingreso de contraseña en la configuración del PAM.
    \end{itemize}
    \item \textbf{REQ\_F\_IUS\_5:}
     \begin{itemize}
        \item Se cumple el requerimiento habiendo especificado el ingreso de contraseña en la configuración del PAM.
    \end{itemize}
    \item \textbf{REQ\_F\_INF\_1:}
     \begin{itemize}
        \item  Configurando el PAM de forma que solo interfiere en el ingreso al servidor vía SSH, se evita impedir el acceso ante una falla mediante formas alternativas.
    \end{itemize}
\end{itemize}
...
Se hace una carpeta \texttt{fastapi-auth} que contendrá el servicio de obtención de claves.
La estructura de dicha carpeta quedará así en esta primera instancia.


\begin{tabular}{ll}
\texttt{app/} & Módulos principales de la aplicación \\
\quad \texttt{main.py} & Punto de entrada del servicio \\
\quad \texttt{\_\_init\_\_.py} & Archivo de inicialización del paquete \\
\texttt{config/} & Archivos de configuración \\
\quad \texttt{settings.yaml} & Parámetros del servicio \\
\texttt{Dockerfile} & Definición de la imagen Docker \\
\texttt{docker-compose.yml} & Orquestación de contenedores \\
...
\begin{itemize}
    ...
    \begin{center}
        \texttt{\char`\<algo\char`\> \char`\<base64-key\char`\> \char`\<comment\char`\>}
    \end{center}
...
\begin{figure}[H]
    ...
    \caption{Arquitectura interna del servicio de autenticación}
...
Se observa que el Proxy SSH interactúa con la capa de presentación, ya que los requests son redirigidos hacia ella a través del proxy. Además, la parte del sistema que interactúa realmente con EJBCA es la capa de infraestructura.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=1.0\linewidth]{fig/structurizr-1-AuthenticationFlow.png}
...
\begin{lstlisting}[language=Bash, caption={Servicio de autenticación principal}, label={cod:authenticate_service}, captionpos=b]
import logging
...
La cobertura de tests alcanzada es del 89\% (638 statements, 67 miss), con porcentajes particularmente altos en los módulos críticos: \textit{application.authenticate\_service} logra 93\%,  
\textit{infrastructure.certificate\_repository\_impl} alcanza 91\%, y \textit{domain.entities.certificate} obtiene 93\%. Los módulos con menor cobertura incluyen \textit{core.config.load\_config} con 25\% y \textit{routes.certificate\_route} con 72\%, que representan deuda técnica manejable dado que la configuración solo se carga al arrancar y las rutas delegan la validación a capas ya testeadas.
...
\begin{lstlisting}[language=Bash, caption={Ejemplos representativos de tests unitarios}, label={cod:representative_unit_tests}, captionpos=b]
...
El pipeline se define en \textit{.github/workflows/auth-server-ci.yml}:
...
\begin{lstlisting}[language=Bash, caption={Respuesta esperada para certificado revocado}, captionpos=b]
Code: 403
Error: Forbidden
Response body:
{
...
\textit{Resultado esperado}:
\begin{lstlisting}[language=Bash, caption={Respuesta esperada para certificado válido}, captionpos=b]
Code: 200
Status: OK
Response body:
{
...
\textit{Resultado esperado}:
\begin{lstlisting}[language=Bash, caption={Respuesta esperada para certificado inexistente}, captionpos=b]
Code: 400
Error: Bad Request
Response body:
{
...
\begin{itemize}
    \item \textbf{REQ\_F\_IUS\_3:}
    \begin{itemize}
        \item El endpoint \texttt{/certificate/\char`\{serial\_id\char`\}/validate} requiere explícitamente el identificador del certificado como parámetro de ruta, formalizando este requerimiento en la interfaz REST.
    \end{itemize}
    \item \textbf{REQ\_F\_INF\_2:}
    \begin{itemize}
        \item El servicio implementa verificación completa de validez mediante consultas a EJBCA para estado de revocación, verificación de fechas de expiración y validación de formato del certificado X.509.
    \end{itemize}
    \item \textbf{REQ\_F\_INF\_4:}
    \begin{itemize}
        \item El sistema registra todas las operaciones de autenticación mediante logging estructurado de FastAPI y logs específicos de cada componente, permitiendo auditoría completa de accesos exitosos y fallidos.
    \end{itemize}
    \item \textbf{REQ\_F\_SYA\_4:}
    \begin{itemize}
        \item La arquitectura garantiza que las claves privadas permanezcan en el host del usuario; el servicio solo maneja claves públicas extraídas de certificados emitidos por EJBCA, eliminando cualquier exposición de material criptográfico sensible.
    \end{itemize}
...
\section{Iteración 7: Actualizar módulo PAM usando servicio de autenticación}
\label{sec:pam_2}
...
La implementación en código consistió en continuar el mismo archivo definido en el bloque de código \ref{cod:modulo_pesonalizado_pam_python_escritura_clave_publica}. Para ello y siguiendo la lógica planteada en la figura \ref{fig:pam_client_sequence} se define una función \texttt{authenticate} que reemplaza la lógica hardcodeada de prueba utilizada en la Iteración 5:

\begin{lstlisting}[language=Bash, caption={Función de comunicación con el servicio de autenticación}, label={cod:pam_authenticate_function}, captionpos=b]
import json
import urllib2

# Funcion para llamar al endpoint del servidor de autenticacion
def authenticate(serial_id, username):
   ...
   if code == 200:
       ...
       print("Error interno en el servidor de autenticacion para el certificado %s" % serial_id)
       return {"allowed": False, "authorized_keys_entry": None}
   else:
       print("Codigo de respuesta inesperado %s para el certificado %s" % (code, serial_id))
       return {"allowed": False, "authorized_keys_entry": None}
\end{lstlisting}

Esta función encapsula toda la comunicación HTTP con el servicio de autenticación, manejando los diferentes códigos de respuesta según la especificación del endpoint definida en la Iteración 6. La función retorna un diccionario con la misma estructura que el \texttt{AuthResponse} del servicio, facilitando su integración con el resto del módulo PAM.

El cambio principal en la función \texttt{pam\_sm\_authenticate} consistió en reemplazar la escritura hardcodeada de la clave pública (líneas del código \ref{cod:modulo_pesonalizado_pam_python_escritura_clave_publica}) por la invocación al servicio real:

\begin{lstlisting}[language=Bash, caption={Integración del servicio en el módulo PAM}, label={cod:pam_service_integration}, captionpos=b]
       def pam_sm_authenticate(pamh, flags, argv):
    try:
        user = pamh.get_user(None)
        if user is None:
        ...
        pamh.conversation(
            pamh.Message(
                pamh.PAM_TEXT_INFO,
                "Certificado encontrado! Clave autorizada anadida a authorized_keys.",
            )
        )
...
El código implementa el flujo de autenticación descrito en la figura \ref{fig:pam_client_sequence}, donde tras obtener el serial del certificado del usuario, se consulta al servicio de autenticación para verificar su validez. En caso de recibir una respuesta positiva, el módulo extrae la clave pública del campo \texttt{authorized\_keys\_entry} y la escribe en el archivo correspondiente del usuario.

La función está contenida en un bloque \texttt{try-catch} general que captura cualquier excepción durante el proceso de autenticación, retornando \texttt{PAM\_USER\_UNKNOWN} para garantizar que errores inesperados no comprometan la estabilidad del sistema.

Esta evolución marca la transición de un módulo PAM de prueba hacia un componente productivo que valida certificados reales contra la infraestructura PKI desplegada. La validación ya no depende de claves estáticas sino que se basa en la verificación dinámica del estado del certificado en EJBCA.
...
Se completaron exitosamente los siguientes requerimientos:

\begin{itemize}
    \item \textbf{REQ\_F\_IUS\_3:}
    \begin{itemize}
        \item El módulo PAM solicita al usuario el número de serie del certificado mediante la interfaz \texttt{pamh.conversation()}, implementando el mecanismo de identificación requerido durante el proceso de autenticación SSH. Y luego puede interpretar el serial que se pidió y se lo pasa al servicio de autenticación para su chequeo.
    \end{itemize}
    
    \item \textbf{REQ\_F\_INF\_2:}
    \begin{itemize}
        \item La función \texttt{authenticate()} consulta al servicio de autenticación que verifica el estado del certificado contra EJBCA, incluyendo validación de vigencia y estado de revocación antes de permitir el acceso al servidor.
        Si es correcta, incluye la clave pública provista por el servidor en el archivo correspondiente para que se continúe el proceso de autenticación en ssh.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item \textit{listen 443 ssl}: Configura Nginx para escuchar en el puerto 443 con soporte SSL/TLS habilitado, estableciendo el punto de entrada seguro para todas las conexiones.
    \item \textit{ssl\_certificate} y \textit{ssl\_certificate\_key}: Definen el certificado del servidor y su clave privada correspondiente que Nginx presenta a los clientes durante el handshake TLS, autenticando la identidad del servicio de autenticación.
    \item \textit{ssl\_client\_certificate}: Especifica el certificado de la CA que debe haber firmado los certificados cliente para considerarlos válidos durante la verificación mTLS.
    \item \textit{ssl\_verify\_client on}: Habilita la verificación obligatoria de certificados cliente, rechazando conexiones que no presenten un certificado válido firmado por la CA especificada.
    \item \textit{proxy\_pass http://fastapi-auth:8888}: Redirige las solicitudes autenticadas al servicio FastAPI ejecutándose en el contenedor interno, actuando como gateway entre el tráfico externo cifrado y el servicio de aplicación interno.
    \item \textit{access\_log /var/log/nginx/access.log}: Registra todas las solicitudes HTTP procesadas por Nginx, incluyendo información sobre verificación de certificados cliente y códigos de respuesta, facilitando la auditoría y debugging del sistema.
\end{itemize}
El código completo es el siguiente:
\begin{lstlisting}[language=Bash, caption={Archivo de configuración .nginx}, label={cod:ngnix_config_file}, captionpos=b]
events {
...
http {
    ...
    server {
        ...
        # Server certificate and key for TLS termination
        ssl_certificate /etc/nginx/ssl/certificate.pem;
        ssl_certificate_key /etc/nginx/ssl/private_key_no_passphrase.key;

        # CA certificate to verify client certificates (mTLS)
        ssl_client_certificate /etc/nginx/ssl/ManagementCA(5).pem;
        ssl_verify_client on;

        location / {
            proxy_pass http://fastapi-auth:8888;
...
\subsubsection{Implementación de mTLS en PAM module}

La implementación continúa sobre la función \texttt{authenticate} definida en el bloque de código \ref{cod:pam_authenticate_function}, agregando únicamente el contexto SSL necesario para establecer autenticación mutua con el servicio. Se requiere importar la librería \texttt{ssl} adicional y modificar la sección de conexión HTTP.

\begin{lstlisting}[language=Bash, caption={Configuración SSL para mTLS en función authenticate}, label={cod:pam_ssl_context}, captionpos=b]
import ssl

# Dentro de la funcion authenticate, antes de urllib2.urlopen:
ctx = ssl.create_default_context(ssl.Purpose.CLIENT_AUTH)

ctx.load_cert_chain(
   certfile='/etc/pam_python/client.pem',
   keyfile='/etc/pam_python/client.key'
)

ctx.load_verify_locations('/etc/pam_python/ca.pem')

request = urllib2.Request(url)
response = urllib2.urlopen(request, context=ctx)
\end{lstlisting}

La configuración del contexto SSL utiliza \texttt{ssl.create\_default\_context} con el propósito \texttt{CLIENT\_AUTH}, estableciendo las bases para la autenticación cliente. Los métodos \texttt{load\_cert\_chain} y \texttt{load\_verify\_locations} cargan respectivamente el certificado cliente con su clave privada y el certificado de la CA para verificar la autenticidad del servidor de autenticación.

Esta implementación garantiza que solo servidores con certificados válidos emitidos por la PKI puedan consultar el servicio de autenticación, eliminando la vulnerabilidad de acceso no autenticado presente en la iteración anterior.
        
\subsubsection{Distribución de certificados y configuración}
\label{sec:certs_for_mtls}
...
\subsubsection{Configuración de Docker Compose con mTLS}

La implementación de mTLS requiere modificar el archivo \texttt{docker-compose.yml} referenciado en el código \ref{cod:docker_compose_servicio} para incluir el servicio Nginx como proxy inverso. La nueva configuración agrega el siguiente bloque:

\begin{lstlisting}[language=Bash, caption={Servicio Nginx en Docker Compose}, label={cod:nginx_docker_compose}, captionpos=b]
nginx:
  image: nginx:alpine
  ports:
    - "443:443"
...
La directiva \texttt{ports} expone el puerto 443 del contenedor, estableciendo nginx como el punto de entrada HTTPS del sistema. Los volúmenes montan la configuración personalizada desde \texttt{.nginx/nginx.conf} (que es el archivo que se describió en \ref{cod:ngnix_config_file} y los certificados desde \texttt{./certs} (descritos en \ref{sec:certs_for_mtls}), ambos en modo de solo lectura para prevenir modificaciones accidentales.

La cláusula \texttt{depends\_on} garantiza que nginx solo inicie después de que FastAPI esté disponible, evitando errores de proxy durante el arranque. Es crítico que todos los archivos de certificados especificados en \texttt{nginx.conf} existan en el directorio \texttt{./certs}, ya que nginx fallará al iniciar si no encuentra algún archivo referenciado en su configuración.

Para levantar el sistema completo con mTLS habilitado se utiliza:

\begin{lstlisting}[language=Bash, caption={Comando para iniciar el servicio con mTLS}, label={cod:iniciar_servicio_mtls}, captionpos=b]
docker-compose up --build
\end{lstlisting}

Este comando construye ambas imágenes (FastAPI y nginx) e inicia los contenedores respetando las dependencias configuradas.
\subsubsection{Pruebas funcionales}
...
En esta iteración se completaron exitosamente los siguientes requerimientos:

\begin{itemize}
    \item \textbf{REQ\_NF\_10:}
    \begin{itemize}
        \item La implementación utiliza exclusivamente herramientas Open Source: Nginx (proxy inverso), OpenSSL (manejo de certificados), Python con \texttt{urllib2} y \texttt{ssl} (cliente), FastAPI (servicio), y Docker (contenedorización), sin depender de productos con licencias comerciales.
    \end{itemize}
    
    \item \textbf{REQ\_NF\_17:}
    \begin{itemize}
        \item El contexto SSL configurado en el módulo PAM establece comunicación TLS cifrada mediante \texttt{ssl.create\_default\_context()}, mientras que la autenticación mutua se logra con \texttt{load\_cert\_chain()} para presentar certificados cliente y \texttt{load\_verify\_locations()} para verificar el servidor.
        Además en la configuración de \texttt{docker-compose.yml} se impide el acceso al servicio de Fast Api de manera directa, haciendo que sólamente se pueda acceder por el puerto \textit{443} con comunicación TLS.
    \end{itemize}
\end{itemize}
...
\section{Iteración 9: Despliegue en entorno de producción}
\label{sec:iteracion_deploy}

\subsection{Antecedente}
Se cuenta con un entorno de producción, que consta de 3 máquinas virtuales conectados entre ellos por redes de distintas VLAN, las cuales cuentan cuentan con SO Ubuntu 20.04, usuarios con directorios propios, y SSH debidamente configurado según requerimientos. Adicionalmente, en uno de ellos se cuenta con un laboratorio de firma digital EJBCA desplegado en docker con base de datos persistente, y configurado para la emisión de certificados X.509.

Por otro lado se cuenta con un entorno de desarrollo, donde se han realizado pruebas sobre el modulo PAM desarrollados con el lenguaje Python. El mismo se comunica un servicio de autenticación (desarrollado en el mismo lenguaje), al cual le envía un identificador de usuario ingresado al momento de autenticación vía SSH, con lo que el servicio valida el certificado al que pertenece, y devuelve la clave pública del usuario si correspondiera. Al finalizar esta comunicación mediante un proxy inverso que asegura una verificación mTLS, la clave pública obtenida es utilizada por PAM para permitir iniciar el proceso de autenticación para clientes SSH mediante el método del par de claves.

\subsection{Desarrollo}

En esta iteración se trabajó exclusivamente en el entorno de producción, trayendo las soluciones desarrolladas por separado.

Para empezar, realizando los pasos indicados en \nameref{sec:mtls} para la configuración de CP \textit{MTLS\_AUTH\_SERVERS} y EEP \textit{SERVER}, se emitieron certificados para los tres servidores desde EJBCA, y se los ubicó en la carpeta \texttt{/usr/lib/security/} mediante el comando \texttt{scp}. Como se explicó en el código \ref{cod:pam_ssl_context}, la configuración del modulo PAM hace referencia a este certificado en formato \textit{pem} y su clave (los cuales se obtienen con los comandos detallados en la sección); para evitar tener que personalizar la configuración del PAM en cada servidor, se utilizaron valores estándar para todos, por ello la ubicación elegida, y como nombre \textit{client.pem} para el certificado, y \textit{client.key} para su clave. También se ubicó el certificado \textit{ca.pem} en el mismo directorio.

Es importante destacar que el primer paso en esta iteración fue la de preparación de certificados que deben ser enviados desde otros servidores, dado que en caso contrario, no sería posible la autenticación con el servidor.

A continuación, en el servidor EJBCA, se descargó el \nameref{sec:github_repo} para desplegar el servicio de autenticación y el proxy inverso. Si bien ambos fueron desarrollados en docker y presentan facilidad para su despliegue se tuvo que adecuar los parámetros al entorno correspondiente.

Se emitió el certificado \textit{localhost.p12} para servidores con \textit{CN} localhost, y se lo guardó en una nueva carpeta de nombre \textit{certificates}. Luego en la misma carpeta se generó el certificado en formato \textit{pem} junto a su clave con nombre \textit{server}, de forma que será lo que se valide con el \textit{client} que se encuentran en los distintos servidores.

Por otro lado, en una nueva carpeta de nombre \textit{certs} se guarda el certificado \textit{SuperAdmin.p12}, así como su clave y formato \textit{pem}.

Para comunicar el servicio de autenticación con EJBCA, se tuvo el cuidado de tener en el docker-compose tanto de EJBCA como del servicio, una red compartida, como se muestra en el código \ref{cod:ejbca_shared_network}

\begin{lstlisting}[language=Bash, caption={Red compartida entre el contenedor de EJBCA y el servicio de autenticación}, label={cod:ejbca_shared_network}, captionpos=b]
networks:
  ejbca_shared_network:
    external: true
\end{lstlisting}

Agregando luego en el apartado de \textit{networks} de fastapi-auth y de EJBCA la red compartida.

Se construyeron nuevamente las imágenes de docker, y se levantaron los contenedores. Previamente a realizarlo en el servicio de autenticación, con la información obtenida de EJBCA con \texttt{docker ps}, en la carpeta \textit{config} se creo \textit{config.prod.yaml} y se detallaron las configuraciones necesarias para la comunicación entre el servicio y EJBCA, como se muestra en el código \ref{cod:config.prod.yaml}.

\begin{lstlisting}[language=Bash, caption={Archivo de configuración que utiliza el servicio de autenticación para comunicarse con EJBCA}, label={cod:config.prod.yaml}, captionpos=b]
ejbca:
  base_url: "https://debian-12-ejbca-1:8443/ejbca/ejbca-rest-api"
  certificate_path: "/code/auth-server/certs/superadmin.pem"
  cert_password: "/code/auth-server/certs/superadmin.key"
  issuer_dn: "CN=PSI-CA"
\end{lstlisting}

Habiendo realizado estos pasos, ya se encuentra levantado el servicio de autenticación comunicado con nuestro EJBCA, por lo que se avanza con el último paso que resta, que es configurar el modulo PAM de todos los servidores.

Siguiendo los pasos detallados en \nameref{sec:pam}, se actualizó la configuración de OpenSSH que habilita el método de autenticación que invoca al modulo PAM y tras instalar \textit{PAM Python} se modificó la autenticación y sesión para que apunten al código personalizado.

El código que se hizo uso, es el desarrollado en \nameref{sec:pam_2}, modificando la linea 6 del código \ref{cod:pam_service_integration} para que apunte al servidor EJBCA, como se puede ver en el código 

\begin{lstlisting}[language=Bash, caption={URL dentro de función authenticate del PAM para que apunte al servidor EJBCA con HTTPS}, label={cod:url_ejbca_pam_https}, captionpos=b]
base_url = "https://10.10.5.2"  # cambiado a HTTPS
\end{lstlisting}

Con las configuraciones ya hechas en el modulo PAM personalizado, y la configuraciones de OpenSSH y usuarios realizado en \nameref{sec:openssh}, se reinicia el servicio de SSH con \texttt{service ssh restart} y quedó finalmente completo todo el sistema.

El último paso es el de ingresar al servidor destino como si se fuera un técnico a través del server Bastion.
En el comando presentado en el código \ref{cod:comando_para_ingreso_por_SSH_con_keypar_en_softhsm} se mostró como apuntar a la clave pública a usar y en el código \ref{cod:comando_conexión_SSH_haciendo_uso_del_servidor_proxy_como_salto} se mostró como es el código para usar un servidor de salto para llegar a otro.

Sin embargo, para la prueba final no se pueden mezclar ambos comandos, dado que el modificador \texttt{-J} solamente permite indicar la clave de la primera conexión, y no la del salto.
Para sortear este inconveniente, se hace uso de un comando compuesto:

\begin{lstlisting}[language=Bash, caption={Ejemplo básico del comando a utilizar para ingresar a un servidor destino a través de un servidor proxy apuntando al par de claves}, label={cod:preset_ssh_final}, captionpos=b]
ssh -i <path_key_destino> -o "ProxyCommand ssh -i <path_key_salto> -W %h:%p <usuario_salto>@<IP_salto>" <usuario_destino>@<IP_destino>
\end{lstlisting}

Donde se debe reemplazar:

\begin{itemize}
    \item \texttt{\char`\<path\_key\_destino\char`\>} $\rightarrow{}$ Debe apuntar a la clave privada que corresponde al certificado a utilizar en el servidor destino.
    \item \texttt{\char`\<path\_key\_salto\char`\>} $\rightarrow{}$ Debe apuntar a la clave privada que corresponde al certificado a utilizar en el servidor proxy.
    \begin{itemize}
        \item Podría existir un caso en que se haga uso de distintas claves y certificados según servidor, en nuestro caso sera el mismo valor en ambos.
    \end{itemize}
    \item \texttt{\char`\<usuario\_salto\char`\>} $\rightarrow{}$ El usuario que se usa en el servidor proxy.
    \item \texttt{\char`\<IP\_salto\char`\>} $\rightarrow{}$ Dirección IP del servidor proxy
    \item \texttt{\char`\<usuario\_destino\char`\>} $\rightarrow{}$ El usuario que se usa en el servidor destino.
    \item \texttt{\char`\<IP\_destino\char`\>} $\rightarrow{}$ Dirección IP del servidor destino que se llega a través del servidor proxy.
\end{itemize}

Finalmente, el comando utilizado para el ingreso al servidor es el mostrado en \ref{cod:ssh_final}.

\begin{lstlisting}[language=Bash, caption={Comando utilizado para ingresar al servidor Destino1 a través del servidor proxy Bastion haciendo uso del par de claves almacenados en SoftHSM}, label={cod:ssh_final}, captionpos=b]
ssh -i /usr/lib/softhsm/libsofthsm2.so -o "ProxyCommand ssh -i /usr/lib/softhsm/libsofthsm2.so -W %h:%p jump@172.18.66.248" destino1@10.10.7.2
\end{lstlisting}

En el proceso de ingreso solicita el identificador del certificado y la contraseña del usuario, en el servidor proxy y en el servidor destino, como se puede ver en la figura \ref{fig:ingreso_ssh}, y finalmente se consiguió ingresar al servidor Destino1.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{fig/ingreso_ssh.png}
    \caption{Solicitud de identificador de certificado y contraseña de usuario en proceso de autenticación a servidor destino pasando por servidor proxy}
    \label{fig:ingreso_ssh}
\end{figure}

En la figura \ref{fig:logs_ssh_1} y \ref{fig:logs_ssh_2} se muestran parte de la salida en consola al ingresar al servidor haciendo uso del modificador \texttt{-vvv} en el proceso de autenticación, para el caso de apuntar a una clave privada dentro de una carpeta que contiene 2 claves públicas diferentes.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{fig/logs_ssh_1.png}
    \caption{Logs extendido durante el proceso de autenticación a un servidor mediante SSH}
    \label{fig:logs_ssh_1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{fig/logs_ssh_2.png}
    \caption{Logs extendido durante el proceso de autenticación a un servidor mediante SSH}
    \label{fig:logs_ssh_2}
\end{figure}

En la figura \ref{fig:logs_ssh_1} se ve primero que informa estar autenticando al usuario por SSH, y que el proceso de autenticación que debe pasar es el \textit{keyboard-interactive}. En este método el usuario no puede ver que internamente el servidor llama al modulo personalizado de PAM, pero si se observa el pedido de ingreso del identificador de certificado presente en el mismo, así como los mensajes avisando que se busca el certificado, se lo encuentra, se lo valida, y finalmente se lo añade a \textit{authorized\_keys}. Posteriormente solicita la contraseña del usuario, y al haberlo ingresado correctamente informa que se ha pasado el proceso de autenticación con éxito parcial, es decir, se ha cumplido con el método \textit{keyboard-interactive}.

En la figura \ref{fig:logs_ssh_2} se muestra que ahora debe pasar por el método de autenticación \textit{publickey}, donde el cliente le envía la clave pública para que el servidor la compare, rechazando la primera que no coincide, y aceptando la segunda que si. Posteriormente recibe un mensaje para ser firmado, y la envía nuevamente al servidor. Finalmente como el proceso fue correcto, indica que la autenticación termino de manera exitosa, y que se ha autenticado al servidor, en este caso, al servidor destino mediante proxy.

En este ultimo log es importante destacar que aunque el servicio de autenticación haya validado el identificador del certificado, y lo haya añadido a la lista de claves autorizadas, el proceso estándar de autenticación del servidor de clave pública sigue vigente, y si el usuario no tiene la clave privada que corresponde al certificado ingresado, no podrá autenticarse al servidor, evitando que el identificador de certificado se convierta en tan solo una contraseña mas, y siga siendo necesario poder demostrar de alguna forma ser propietario del certificado para autenticarse.

Con el proceso validado, se trabajó en facilitar el proceso de ingreso, dado que se entiende que es engorroso y complicado, tanto el largo del comando, como tener que copiar el identificador del certificado y la contraseña. Para ello, se desarrolló el script \ref{cod:ssh_connect.sh} para bash.

\begin{lstlisting}[language=Bash, caption={Script de bash para simplificar el ingreso a servidores}, label={cod:ssh_connect.sh}, captionpos=b]
#!/usr/bin/expect -f

# Set variables
set timeout -1
set jump_host "<usuario_salto>@<IP_salto>"
set target_host "<usuario_destino>@<IP_destino>"
set serial_number "<serial_id>"
set cert_key "<path_key>"
set password1 "<pass_usuario_salto>"
set password2 "<pass_usuario_destino>"

# Start SSH session
spawn ssh -i $cert_key -o "ProxyCommand ssh -i $cert_key -W %h:%p $jump_host" $target_host

# Handle prompts
expect "Ingrese el serial_id de su certificado:"
...
Se generó el código \ref{cod:ssh_connect.sh} dentro de un nuevo archivo, donde los valores a modificar son los mismos que los presentados en el comando \ref{cod:preset_ssh_final}, a los que se le suman:

\begin{itemize}
    \item \texttt{\char`\<serial\_id\char`\>} $\rightarrow{}$ El valor del identificador del certificado.
    \item \texttt{\char`\<path\_key\char`\>} $\rightarrow{}$ Debe apuntar a la clave privada que corresponde al certificado. 
    \item \texttt{\char`\<pass\_usuario\_salto\char`\>} $\rightarrow{}$ La contraseña del usuario que se especifico para el servidor proxy.
    \item \texttt{\char`\<pass\_usuario\_destino\char`\>} $\rightarrow{}$ La contraseña del usuario que se especifico para el servidor destino.
\end{itemize}

En un caso real, un técnico podría necesitar ingresar a distintos servidores, por lo que este script con las variables fácilmente identificables al inicio, permite tener varias copias con los valores correspondientes a los distintos destinos, y nombrar cada uno con un nombre descriptivo, como por ejemplo \textit{destino1.sh}, con lo que sabría es el que debe utilizar para ingresar al servidor Destino1, y luego tener otro llamado \textit{destino2.sh} que sera el que use para ingresar al servidor Destino2.

Para hacer uso de este script, se debe haber con el comando \texttt{expect \char`\<path\_script\char`\>}, donde \textit{\textless{}path\_key\textgreater{}} es la ubicación del script.

Si se necesitará recuperar los logs del proxy inverso o del servicio de autenticación, se pueden ver ingresando al servidor EJBCA donde se encuentran corriendo, y tras utilizar el comando \texttt{docker ps} se usa su identificador (por ejemplo 71d8a9a5aaa3 para el contenedor de nginx o f6b307536fc0 para el servicio de autenticación) en el comando \texttt{docker logs -f 71d8a9a5aaa3} ó \texttt{docker logs -f f6b307536fc0}, y se tendra acceso a sus logs pasados, y a los que se generan en tiempo real.

Con nuestro sistema ya completo, se realizan pruebas de conexión al servidor destino para corroborar la respuesta a conexiones exitosas vía SSH con la carga extra que implican las configuraciones realizadas.

\begin{itemize}
    \item Se cronometran 10 accesos individuales al servidor, y saca un valor promedio de 2,2 segundos.
    \item Se genera un script que lanza 50 veces en simultaneo el ingreso a servidores, y tras finalizar, se puede observar en VMware en la página de la VM que contiene un gráfico con el rendimiento del sistema en la ultima hora en porcentajes, un aumento del consumo de memoria RAM de 0.1GB (de 0.8 a 0.9), mientras que el procesador tuvo un incremento del 1\% al 15\%
\end{itemize}


\subsection{Conclusión}

Al finalizar esta iteración se juntaron todas las piezas trabajadas por separadas en el proyecto, generando un gran sistema, donde existe una CA capaz de emitir y validar certificados X.509, la cual es consultada por un servicio de autenticación protegido por un proxy inverso que aplica mTLS, con el objetivo de validar los certificados de usuarios, que al tratar de ingresar a un servidor, el proceso de autenticación se comunica con el servicio a través de un modulo PAM personalizado, que con un identificador de certificado que ingresa el usuario, consulta por su validez y en caso de serlo solicita su clave pública, la cual ingresará posteriormente a la lista de claves públicas habilitadas para la autenticación con el método del par de claves.

Todo este proceso realizado sobre un sistema que consiste de un servidor proxy y un servidor destino al que se quiere ingresar, se encuentran de esta forma restringidos en su acceso mediante SSH al solicitar acceso al servidor haciendo uso de manera indirecta de un certificado X.509 vigente.

Habiendo terminado esta iteración, vale destacar el cumplimiento de los siguientes requerimientos y su forma:

\begin{itemize}
    \item \textbf{REQ\_F\_SYA\_1:} El acceso a un servidor no deberá permitir el ingreso a otros servidores.
    \begin{itemize}
        \item 
    \end{itemize}
    \item \textbf{REQ\_F\_INF\_2:} 
    \begin{itemize}
        \item 
    \end{itemize}
    \item \textbf{REQ\_F\_INF\_3:}
    \begin{itemize}
        \item Para la verificación de validez de un certificado, el modulo PAM consulta a un servicio de autenticación que se encuentra por fuera del servidor en tiempo real, es decir, en el momento que se esta solicitando acceder al servidor. De esta forma, no es necesario realizar ninguna modificación en ningún servidor para permitir o quitar el acceso en base a la emisión o revocación de certificados.
    \end{itemize}
    \item \textbf{REQ\_NF\_1:} 
    \begin{itemize}
        \item Una vez todas las configuraciones fueron realizadas, para conectarse a un servidor solamente es necesario utilizar un comando, y en caso de necesitar realizar una modificación, tan solo debe modificar unos valores en un archivo de texto plano bien identificados.
    \end{itemize}
    \item \textbf{REQ\_NF\_3:} 
    \begin{itemize}
        \item Una vez el sistema estuvo completo, se realizar pruebas varias de conexión confirmando un tiempo de respuesta menor a los 3 segundos.
    \end{itemize}
    \item \textbf{REQ\_NF\_5:} 
    \begin{itemize}
        \item Las configuraciones realizadas tanto en OpenSSH como en PAM son compatibles en las versiones de Ubuntu 16.04.07 hasta las versiones más modernas.
    \end{itemize}
    \item \textbf{REQ\_NF\_6:} 
    \begin{itemize}
        \item Las configuraciones a realizar en el equipo del usuario, consisten en una lista acotadas de comandos, sin grandes tiempo de carga que permiten poder ser realizadas en menos de media hora.
    \end{itemize}
    \item \textbf{REQ\_NF\_7:} 
    \begin{itemize}
        \item Con el sistema de verificación y validación de certificados en un servicio por fuera de los servidores al que se comunica cada servidor de manera particular, con el que se usa su respuesta al momento de intentar ingresar al servidor para validar el certificado del usuario, aunque se duplique la cantidad de servidores o de usuarios, como no es necesaria intervención manual alguna en el proceso, es totalmente escalable.
    \end{itemize}
    \item \textbf{REQ\_NF\_8:} 
    \begin{itemize}
        \item En el documento se han descrito todas las configuraciones necesarias, así como indicar repositorios en los que apoyarse, con lo que podría ser replicado el sistema en su totalidad.
    \end{itemize}
    \item \textbf{REQ\_NF\_11:} 
    \begin{itemize}
        \item Las modificaciones realizadas en el servidor proxy y destino, constan de la modificaciones realizadas en las configuración de OpenSSH y PAM, ademas de los certificados y clave para hacer el handshake con el mTLS del servicio de autenticación. En total, esto consta de menos de 60KB. 
    \end{itemize}
    \item \textbf{REQ\_NF\_13:} 
    \begin{itemize}
        \item Se hicieron pruebas de conectar 50 usuarios al mismo tiempo, y si bien se elevaron los recursos consumidos por las VM, pudieron completarse, y el sistema pudo seguir operando con normalidad.
    \end{itemize}
\end{itemize}